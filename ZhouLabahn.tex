...
\title{Computing Column Bases of Polynomial Matrices}
\maketitle
\begin{abstract}
...
We will need the following result from \cite{za2012} to bound the
...
Let 
\[
\mathbf{N}=\left[\mathbf{N}_{1},\mathbf{N}_{2},\cdots,\mathbf{N}_{\left\lceil \log k\right\rceil -1},\mathbf{N}_{\left\lceil \log k\right\rceil }\right]
\]
...
the same dimension as the row dimension of $\mathbf{N}_{i}$ and $\vec{\sigma}=\left[\vec{\sigma}_{1},\vec{\sigma}_{2},\dots,\vec{\sigma}_{\left\lceil \log k\right\rceil }\right]$
...
\subsection{Cost of Left Kernel Basis Computation}
...
\[
\left[V^{T},\bar{\mathbf{T}}^{T}\right]=\begin{bmatrix}V\\
\bar{\mathbf{T}}
\end{bmatrix}^{T}=\left[\begin{array}{cccc}
...
\[
\mathbf{T}=\left(V^{T}\right)^{-1}\bar{\mathbf{T}}^{T}=\left[\begin{array}{cc}
x+x^{2} & 1\\
1+{x}^{2} & 1
\end{array}\right].
\]
...
\item The condition $\sum\vec{t}^{*}\le\xi$ follows from \prettyref{lem:colBasisdegreeBoundByRdegOfRightFactor}. 
...
basis $\begin{bmatrix}V^{T},\bar{\mathbf{T}}^{T}\end{bmatrix}^{T}$
computed, a column basis is then given by $\mathbf{T}~=~\left(\bar{\mathbf{T}}V^{-1}\right)^{T}$.
