...
a column basis of $\mathbf{F}$. We represent column bases using matrices,
with columns being the basis elements.
\begin{exmp}
\label{exm:columnBasis}Let $\mathbb{K}=\mathbb{Z}_{2}$ and 
\[
...
\end{array}\right]
\]
 is a column basis of $\mathbf{F}$ that generates the same module
as the columns of $\mathbf{F}$.
\end{exmp}
When the row dimension $m=1$, a column basis becomes the standard
polynomial GCD. GCD is one of the most fundamental subject in algebra
and computer algebra and has been extensively studied. As a generalization
of polynomial GCD and matrix GCD, column bases are also fundamental
in polynomial matrix computations. Column basis computation provides
a deterministic alternative to randomized lattice compression \citep{li:2006,storjohann-villard:2005}.
...
possible. In \prettyref{exm:columnBasis}, the matrix 
\[
...
\end{array}\right]
\]
 is a column reduced form of $\mathbf{F}$, and its submatrix 
\[
\begin{bmatrix}x & 1\\
1 & 1
\end{bmatrix}
\]
 consists of its nonzero columns is a column basis with the smallest
possible column degrees. The Popov form is a special unique column
reduced form satisfying additional conditions. The Hermite normal
form provides another special unique column basis that is triangular
and has a different type of minimal column degrees.
...
\section{From Kernel Basis to Column Basis}
...
\end{array}\right]
\]
 is a right kernel basis of $\mathbf{F}$. The matrix 
\[
...
\end{array}\right]
\]
 is a left kernel basis of $\mathbf{N}$. Then the matrix 
\[
...
\end{array}\right]
\]
{\tiny{} }satisfies $\mathbf{F}=\mathbf{T}\mathbf{G}$, and is a column
basis of $\mathbf{F}$.
\end{exmp}
...
basis to be the same degree as $I$.\end{proof}
\begin{exmp}
Let 
\[
\mathbf{F}=\left[\begin{array}{cccc}
...
\end{array}\right]
\]
be a left kernel basis of a right kernel basis of $\mathbf{F}$. To
compute the column basis $\mathbf{T}$ satisfying $\mathbf{F}=\mathbf{T}\mathbf{G}$,
first we can determine $\cdeg\mathbf{T}\le\vec{t}=\left[2,0\right]$
from \prettyref{lem:colBasisdegreeBoundByRdegOfRightFactor}. Then
...
\end{array}\right]
\]
 is such a left kernel basis. Then a column basis can be computed
by $\mathbf{T}=V^{-1}\bar{\mathbf{T}}=\left[\begin{array}{cc}
x+x^{2} & 1\\
1+{x}^{2} & 1
\end{array}\right]$.
\end{exmp}
